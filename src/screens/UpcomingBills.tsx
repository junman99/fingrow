    ...
    <ScreenScroll contentStyle={{ paddingBottom: spacing.s24 }}>
      <View style={{ paddingHorizontal: spacing.s16, paddingTop: spacing.s12, gap: spacing.s16 }}>
        ...
        <View style={{
          ...
          <Text style={{ color: warningColor, fontWeight: '700', fontSize: 12, textTransform: 'uppercase', letterSpacing: 0.5 }}>
            Total held
          </Text>
          <Text style={{ color: textPrimary, fontSize: 42, fontWeight: '900', letterSpacing: -1.5 }}>
            {fmtMoney(holdAmount)}
          </Text>
          ...
          <Text style={{ color: textMuted, fontSize: 14 }}>
            Due before {endOfDay(period.end).toDateString()}
          </Text>
        </View>
        ...
        {holdItems.length === 0 ? (
          <View style={{
            ...
            <View style={{
              ...
              <Icon name="calendar" size={32} color={textMuted} />
            </View>
            <Text style={{ color: textPrimary, fontWeight: '700', fontSize: 18 }}>No upcoming bills</Text>
            <Text style={{ color: textMuted, textAlign: 'center' }}>
              Log your recurring bills to protect cash and track spending better.
            </Text>
            <Button title="Add recurring bill" onPress={() => nav.navigate('Bills')} style={{ marginTop: spacing.s8 }} />
          </View>
        ) : (
                ...
                <View
                  ...
                  style={{
                    ...
                    borderColor: isUrgent
                      ...
                      : withAlpha(borderSubtle, isDark ? 0.5 : 1),
                    gap: spacing.s10
                  }}
                >
                  <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <View style={{ flex: 1, paddingRight: spacing.s12 }}>
                      <Text style={{ color: textPrimary, fontWeight: '700', fontSize: 16 }} numberOfLines={1}>
                        {it.label}
                      </Text>
                      {it.category ? (
                        <Text style={{ color: textMuted, fontSize: 13, marginTop: 2 }}>{it.category}</Text>
                      ) : null}
                    </View>
                  ...
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: spacing.s8 }}>
                    <Icon name="calendar" size={14} color={isUrgent ? warningColor : textMuted} />
                    <Text style={{
                      color: isUrgent ? warningColor : isClose ? textPrimary : textMuted,
                      fontSize: 13,
        ...
        {/* Manage Button */}
        {holdItems.length > 0 ? (
          <Button title="Manage recurring bills" variant="secondary" onPress={() => nav.navigate('Bills')} />
        ) : null}
      </View>
    </ScreenScroll>
  );
